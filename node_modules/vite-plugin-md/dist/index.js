...
var baseStyling = transformer("baseStyling", "metaExtracted", "metaExtracted", (p2) => {
  ...
  if (p2.options.style.baseStyle === "github") {
    const style = createInlineStyle().addCssVariable("md-text-color", "#111827").addCssVariable("md-text-color", "#e5e7eb", "html.dark").addCssVariable("md-code-background", "rgba(27,31,35,.05)").addCssVariable("md-code-background", "rgba(229, 231, 235, 0.65)", "html.dark").addCssVariable("code-font", 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace').addClassDefinition(
...
var createParser = transformer("createParser", "metaExtracted", "parser", (payload) => {
  var _a2;
  const parser = new MarkdownIt(__spreadValues({
    html: true,
    linkify: true,
    typographer: true
...
var escapeCodeTagInterpolation = transformer(
  ...
  (payload) => {
    ...
    const html = usingBuilder("code") ? dom : select(dom).updateAll("pre")((pre) => {
...
import { extract, getAttribute, select as select2, toHtml } from "@yankeeinlondon/happy-wrapper";
...
function extractScriptBlocks(p2) {
  ...
  const html = select2(p2.html).updateAll("script")(extractor).toContainer();
  p2.html = html;
  const lang = getAttribute("lang");
  const scriptSetups = select2(scripts.join("\n")).findAll("script[setup]");
 ...
 - [ file: ${p2.fileName}, node: ${toHtml(ss)} ]`);
  });
  const traditionalScripts = select2(scripts.join("\n")).findAll("script:not([setup])");
  return { scriptSetups, traditionalScripts };
}
function extractCustomBlocks(p2, options) {
  const styleBlocks = [
    ...select2(p2.html).findAll("style"),
    ...elementHashToArray(p2.vueStyleBlocks)
  ];
  const html = select2(p2.html).updateAll("style")(extract());
  let customBlocks = [];
  for (const tag of options.customSfcBlocks) {
    const found = select2(p2.html).findAll(tag);
...
function generateSourceMap(filename, source, generated, parser) {
  const map4 = new SourceMapGenerator({
...
var pipelineUtilityFunctions = (ctx) => ({
  ...
  addScriptReference(script) {
    ...
    add2(ctx.head.script, isElement(script) ? convertToDictionary(script) : script);
  },
  ...
  addStyleBlock(name, style) {
    ctx.vueStyleBlocks[name] = typeof style === "string" ? createElement(style) : style;
  },
...
import { clone, select as select3 } from "@yankeeinlondon/happy-wrapper";
...
var repairFrontmatterLinks = transformer("repairFrontmatterLinks", "parsed", "parsed", (payload) => {
  ...
  payload.html = select3(payload.html).updateAll("img")(update("src")).updateAll("iframe")(update("src")).updateAll("srcset")(update("src")).updateAll("a")(update("href")).updateAll("link")(update("href")).updateAll("prefetch")(update("href")).toContainer();
  return payload;
});
