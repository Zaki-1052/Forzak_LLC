class m{constructor(){this.cache=new Map}async loadContent(t){var e;try{console.log(`🔄 Starting to load content: ${t}`);const o=`/content/${t}.md`;console.log(`📡 Fetching from URL: ${o}`);const n=await fetch(o);if(console.log("📡 Fetch response:",{ok:n.ok,status:n.status,statusText:n.statusText,url:n.url,headers:Object.fromEntries(n.headers.entries())}),!n.ok)throw console.error(`❌ Fetch failed with status ${n.status}: ${n.statusText}`),new Error(`Failed to fetch ${t}.md: ${n.status} ${n.statusText}`);const i=await n.text();console.log("📄 Raw markdown loaded:",{length:i.length,preview:i.substring(0,300)+"...",firstLine:i.split(`
...
//# sourceMappingURL=contentLoader-CW06xALV.js.map
